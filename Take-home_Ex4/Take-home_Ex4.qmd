...
Our column of time is formatted with YYYY Month. Now we need to separate the year and month with a function as shown below.

```{r}
mak_month<-function(ts){
...
```{r}
month_level<-c("Jan, Feb, March, April, May, June, July, Aug, Sep, Oct, Nov, Dec")
...
parse_date(imp$Month)
imp<-imp%>%
  mutate(Month = recode(AG, ""))
```
