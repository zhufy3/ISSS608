...
![](images/paste-6CEDCAD8.png){width="491"}
...
![](images/paste-EBFE4AA4.png){height="500"}
...
![](images/paste-8DE01A88.png){style="width " width="810"}
...
![](images/paste-BDFF6A0A.png){width="810"}
...
![](images/paste-447F4895.png){width="810"}
...
![](images/paste-A4212DA8.png){width="810"}
...
![](images/paste-26687E54.png){width="810"}
...
![](images/paste-3BA2819D.png){width="810"}
...
### ![](images/paste-A57D5DFE.png){width="810"}
...
![](images/paste-7F15D709.png){width="810"}
...
```{r}
sdata<-data%>%
  select(c("town","flat_type"))%>%
  group_by(town)%>%
  summarise(`N 4ROOM` = n("flat_type"="4 ROOM"))



funnel_plot(
  numerator = data$floor_area_sqm,
  denominator = data$resale_price,
  group = data$street_name)
```
...
```{r}
leaflet(geodata) %>% addTiles() %>% addProviderTiles("OneMapSG") %>% addMarkers(~long, ~lat, popup = ~address, label = ~round(resale_pricePersqm), clusterOptions = markerClusterOptions())
```
