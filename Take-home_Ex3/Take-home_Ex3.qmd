...
Also it shows that it's good to use non-parametric test because it's better to use ***medium*** to represent our sample. Even though we have large enough data set, we don't assume the distribution of resale price per sqm conforming to normal distribution, that's why we use ***non-parametric*** test.
...
Noted that these three distributions don't fit normal distributions. Therefore, let's test their distributions with ***non-parametric*** **test.**
...
From the plot above, noted that variables *flat_model, flat_type* and *floor_area_sqm* have highly collinearity. While variables *lease_commence_date* and *storey_range* have low colliearity.
