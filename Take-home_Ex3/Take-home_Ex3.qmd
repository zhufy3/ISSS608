...
```{r}
pacman::p_load(tidyverse, tidyr, ggstatsplot, funModeling, plotly, ungeviz, gganimate, DT, patchwork, ggdist)
```
...
![](images/paste-8DE01A88.png)As shown in plot above, we see that these two variables have high association. Therefore, we can just delete one column of them. Let us delete the column named *remaining_lease.*
...
t1<-datatable(sum_tbl, options = list(pageLength = 12))
